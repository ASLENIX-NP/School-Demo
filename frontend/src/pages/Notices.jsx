...
export function sortNoticesNewestFirst(list = []) {
  return [...list].sort((a, b) => {
    ...
    const da = new Date(a.notice_date || a.created_at || 0).getTime();
    const db = new Date(b.notice_date || b.created_at || 0).getTime();
    return db - da;
  });
}
    ...
    >
      ...
      >
        <div
          ...
          style={{
            background: `linear-gradient(90deg, ${color}, #D9A441, #4E9AA8)`,
          }}
        />
        ...
        {editMode && (
          <div className="absolute right-4 top-4 z-20 flex gap-2">
            <button
              ...
              className="rounded-xl bg-slate-950 px-3 py-2 text-xs font-black text-white shadow-lg hover:-translate-y-0.5"
            >
              Edit
            </button>
            <button
              ...
              className="rounded-xl bg-red-50 px-3 py-2 text-xs font-black text-red-600 shadow-lg hover:-translate-y-0.5"
            >
              Delete
            </button>
          </div>
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="shrink-0 text-right text-xs font-bold text-slate-400">
            <CalendarDays size={15} className="ml-auto mb-1" />
...
export default function Notices({
  ...
  useEffect(() => {
    if (editMode) {
      setNotices(sortNoticesNewestFirst((noticesOverride || []).map(normalizeNotice)));
      setAnnouncements(announcementsOverride || []);
      setSettings({
    ...
    async function load() {
      setLoading(true);
      try {
        const [noticeResult, announcementResult, settingsResult] =
          await Promise.all([
            fetchJson(`${API_URL}/api/notices`),
            fetchJson(`${API_URL}/api/announcements`),
            fetchJson(`${API_URL}/api/notice-settings`).catch(() => null),
          ]);

        if (!alive) return;

        const rawNotices = Array.isArray(noticeResult)
          ? noticeResult
          : noticeResult?.data || [];

        const rawAnnouncements = Array.isArray(announcementResult)
          ? announcementResult
          : announcementResult?.data || [];

        setNotices(sortNoticesNewestFirst(rawNotices.map(normalizeNotice)));

        setAnnouncements(
          rawAnnouncements
            .filter((item) => item?.active !== false && item?.visible !== false)
            .sort((a, b) => {
              if (Boolean(a.show_on_homepage) !== Boolean(b.show_on_homepage)) {
                return a.show_on_homepage ? -1 : 1;
              }
              return (
  ...
  const filteredNotices = useMemo(() => {
    ...
    return notices.filter((notice) => {
      ...
      const matchesPdf =
        !pdfOnly || Boolean(notice.pdf_url || notice.file_url);

      return matchesSearch && matchesCategory && matchesPdf;
    });
  }, [notices, query, category, pdfOnly]);

  const pinned = filteredNotices.filter((item) => item.pinned);
  const regular = filteredNotices.filter((item) => !item.pinned);

  const importantCount = notices.filter((item) => item.pinned).length;
  const pdfCount = notices.filter(
    (item) => item.pdf_url || item.file_url
  ).length;
    ...
    >
      <section className="relative px-4 pb-14 pt-24 sm:px-6 lg:px-8">
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          >
            {editMode && (
              <EditBadge onClick={() => onEditTarget({ type: "pageHeader" })}>
                Edit Heading
              </EditBadge>
            )}
            ...
            <div className="relative max-w-3xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur">
                  <div className="text-2xl font-black">{notices.length}</div>
                  <div className="text-xs font-bold text-white/65">
                    Official notices
                  </div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur">
                  <div className="text-2xl font-black">{importantCount}</div>
                  <div className="text-xs font-bold text-white/65">
                    Important
                  </div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur">
                  <div className="text-2xl font-black">{pdfCount}</div>
                  <div className="text-xs font-bold text-white/65">
                    Documents
                  </div>
                </div>
        ...
        <div className="mx-auto max-w-7xl">
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map((item) => (
                  <button
                    ...
                    style={{
                      background:
                        category === item ? "#173E2D" : "#F4F7F5",
                      color: category === item ? "#FFFFFF" : "#64748B",
                    }}
                  >
                    {item}
                  </button>
                ...
                <button
                  ...
                  style={{
                    background: pdfOnly ? "#DFF5EE" : "#F4F7F5",
                    color: pdfOnly ? "#2D6A4F" : "#64748B",
                  }}
                >
          ...
          {editMode && (
            <div className="mb-7 flex flex-wrap gap-3 rounded-[24px] border border-emerald-200 bg-emerald-50 p-4">
              <button
                type="button"
                onClick={onAddNotice}
                className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-3 text-sm font-black text-white shadow-lg"
              >
                + Add Notice
              </button>
              <button
                type="button"
                onClick={() => onEditTarget({ type: "sidebar" })}
                className="rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-700 shadow-sm ring-1 ring-slate-200"
              >
                Edit Info Card
              </button>
              <button
          ...
          {loading ? (
            <div className="rounded-[30px] bg-white p-16 text-center shadow-xl">
              ...
              <p className="mt-4 font-bold text-slate-500">Loading notices...</p>
            </div>
          ) : regular.length === 0 ? (
            <div className="rounded-[30px] border border-dashed border-slate-300 bg-white/80 p-16 text-center">
              ...
              {editMode && (
                <button
                  type="button"
                  onClick={onAddNotice}
                  className="mt-6 rounded-xl bg-emerald-700 px-5 py-3 text-sm font-black text-white"
                >
                  Add First Notice
                </button>
              )}
          ...
          {!editMode && (
            <div
              className="mt-12 relative overflow-hidden rounded-[30px] p-7 text-white shadow-2xl sm:p-9"
              style={{
                background:
            ...
            >
              ...
              <div className="relative flex flex-col justify-between gap-6 md:flex-row md:items-center">
                ...
                >
                  {settings.sidebar_button_text || "View Calendar"}
            ...
            >
              ...
              <div className="p-6 sm:p-9">
                <button
                  type="button"
                  onClick={() => setSelectedNotice(null)}
                  className="float-right rounded-full bg-slate-100 p-3 text-slate-500 transition hover:rotate-90 hover:bg-slate-200"
                >
                  <X size={18} />
                </button>
                  ...
                  <div className="mt-8 rounded-2xl bg-slate-50 p-5 ring-1 ring-slate-200">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="font-black text-slate-900">Attached document</div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-3">
                      ...
                      <a
                        href={selectedNotice.pdf_url || selectedNotice.file_url}
                        download
                        className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-700 ring-1 ring-slate-200"
                      >
