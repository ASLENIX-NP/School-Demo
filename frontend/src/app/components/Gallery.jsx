...
/*
|--------------------------------------------------------------------------
| RED ROSE SCHOOL — GALLERY
|--------------------------------------------------------------------------
| Replacement for the existing Gallery.jsx.
|
...
const FALLBACK_DESCRIPTIONS = {
  ...
  Events:
    "School events capture celebrations, competitions, cultural programs, student participation, and memorable school occasions.",
  Certificate:
    "Certificates and awards recognize achievement, participation, discipline, excellence, and student accomplishments.",
};
...
const DEFAULT_ACHIEVEMENTS = [
  ...
  { id: "4", title: "Community Service", year: "2023", icon: "Award" },
];
...
function normalizeSubcategories(value, categories) {
  return categories.reduce((result, category) => {
    const items = Array.isArray(value?.[category])
      ? value[category]
      : [];
...
function HighlightedTitle({ title, highlightedText }) {
  ...
  return (
    <>
      {before}
      <span className="gallery-gold-text">
        {highlightedText}
      </span>
      {after}
    </>
...
function ZigZag({ color = COLORS.cream }) {
  return (
    <div
      className="gallery-zigzag"
      style={{
        background: color,
        clipPath:
          "polygon(0 42%, 4% 100%, 8% 42%, 12% 100%, 16% 42%, 20% 100%, 24% 42%, 28% 100%, 32% 42%, 36% 100%, 40% 42%, 44% 100%, 48% 42%, 52% 100%, 56% 42%, 60% 100%, 64% 42%, 68% 100%, 72% 42%, 76% 100%, 80% 42%, 84% 100%, 88% 42%, 92% 100%, 96% 42%, 100% 100%, 100% 100%, 0 100%)",
      }}
    />
  );
}
...
function GalleryHero({
  ...
  const highlight =
    content.heroHighlightedText || "Memories";
    ...
    <section className="gallery-hero">
      ...
      <div className="gallery-hero-inner">
        <motion.div
          ...
          className="gallery-label gallery-label-light"
        >
          <span />
          {content.heroBadge}
        </motion.div>
      ...
      <div className="gallery-hero-mark" aria-hidden="true">
        <div className="gallery-hero-mark-inner">
          <strong>RR</strong>
          <span>RED ROSE</span>
        </div>
      </div>
    ...
    <section className="gallery-achievements">
      <div className="gallery-shell">
        <div className="gallery-section-heading">
          ...
          <h2>
            Celebrating <em>Excellence</em>
          </h2>

          <p>
            Every certificate, competition, and achievement represents
            a student who chose to learn, participate, and grow.
          </p>
        </div>
...
function createAlbums(content, activeCategory) {
  ...
  if (activeCategory === "All") {
    return content.categories
      .map((category) => makeAlbum(category))
  ...
  if (!subs.length) {
    return [makeAlbum(activeCategory)].filter(
  ...
  const albums = subs
    .map((item) =>
      makeAlbum(activeCategory, item.name)
    )
    .filter((album) => album.photos.length > 0);
  ...
  if (
    ...
    !albums.some((album) => album.photos.length)
  ) {
    albums.push(general);
  }
      ...
      <div className="gallery-card-image">
        ...
        {album.photos.length > 1 && (
          <div className="gallery-slide-dots">
            {album.photos.slice(0, 5).map((_, dot) => (
              <span
                key={dot}
                className={
                  dot === slide ? "active" : ""
...
function GalleryViewer({
  ...
  const download = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download =
        `${photo.title || "red-rose-gallery"}-${imageIndex + 1}.jpg`
          .replace(/\s+/g, "-")
          .toLowerCase();
      ...
      >
        ...
        >
          ...
          <div className="gallery-viewer-photo">
            <button
              type="button"
              className="gallery-viewer-photo-button"
              onClick={() => {
                setZoomOpen(true);
                setZoom(1);
              }}
            ...
            <div className="gallery-viewer-counter">
              {imageIndex + 1} / {album.photos.length}
            </div>
          </div>

          <div className="gallery-viewer-info">
            <div className="gallery-viewer-label">
              {album.category}
              {album.subcategory && (
                <>
                  <span>•</span>
        ...
        {zoomOpen && (
          <motion.div
            className="gallery-zoom-backdrop"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
  ...
  return (
    <main className="red-rose-gallery">
      ...
      <section className="gallery-closing">
        ...
        <div className="gallery-closing-inner">
          <Sparkles size={21} />
          ...
          <p>
            Learning, friendship, achievement, and
      ...
      <style>{`
        ...
        .gallery-hero-glow-two {
          ...
          right: -160px;
          bottom: 30px;
          background: rgba(166,43,79,.16);
          box-shadow: 0 0 130px rgba(166,43,79,.18);
        }
        ...
        .gallery-gold-button {
          ...
          cursor: pointer;
          box-shadow:
            0 10px 28px rgba(199,154,59,.18);
          transition:
            transform .25s ease,
            box-shadow .25s ease;
        }
        ...
        .gallery-gold-button:hover {
          ...
          box-shadow:
            0 15px 35px rgba(199,154,59,.25);
        }
        ...
        .gallery-hero-mark-inner {
          ...
          align-items: center;
          justify-content: center;
          border: 1px dashed rgba(229,200,120,.65);
          border-radius: 50%;
          background: #2b1622;
        }

        .gallery-hero-mark strong {
          color: #e6c978;
          font-family:
            Georgia,
            "Times New Roman",
        ...
        .gallery-heading-divider span {
          width: 45px;
          height: 1px;
          background: #d6c8bb;
        }

        .gallery-heading-divider i {
          width: 6px;
          height: 6px;
          border-radius: 50%;
          background: var(--gold);
        ...
        .gallery-card-top span {
          color: white;
          background: rgba(36,19,31,.55);
          border: 1px solid rgba(255,255,255,.17);
        }

        .gallery-card-top b {
          color: #38271d;
          background: rgba(229,200,120,.93);
        }
        ...
        .gallery-slide-dots span.active {
          width: 15px;
          border-radius: 99px;
          background: white;
        }
        ...
        .gallery-card:hover .gallery-open-button {
          opacity: 1;
          transform:
            translate(-50%, -50%)
            scale(1);
        ...
        .gallery-closing-inner svg {
          color: var(--gold-light);
        }

        .gallery-closing-inner h2 {
          margin: 12px 0 8px;
          font-family:
            Georgia,
            "Times New Roman",
        ...
        @media (prefers-reduced-motion: reduce) {
          .red-rose-gallery *,
          .red-rose-gallery *::before,
          .red-rose-gallery *::after {
