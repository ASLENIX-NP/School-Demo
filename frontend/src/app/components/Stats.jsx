...
function TiltCard({ children, className = "", max = 8, style = {}, editMode = false, ...props }) {
...
// ─── 2. DYNAMIC FLOATING BACKGROUND PARTICLES (Public only) ───
function FloatingBackground() {
  const shapes = Array.from({ length: 12 }).map((_, i) => ({
    ...
    duration: 18 + (i % 5) * 4,
    delay: (i * 1.5) % 8,
    color: i % 2 === 0 ? "rgba(233, 196, 106, 0.15)" : "rgba(30, 58, 95, 0.10)"
  }));
...
export default function Stats({
  ...
  if (loading) {
    return (
      <div className={`flex items-center justify-center bg-slate-50 text-slate-900 ${editMode ? "py-12" : "min-h-screen"}`}>
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-4 border-indigo-400/30 border-t-indigo-500 rounded-full animate-spin" />
          <p className="text-xs font-bold tracking-widest text-slate-400">LOADING CONTENT</p>
        </div>
      </div>
    );
  }
  ...
  if (!data || !data.stats || data.stats.length === 0) {
    return (
      <div className={`flex items-center justify-center bg-slate-50 text-slate-900 ${editMode ? "py-12" : "min-h-screen"}`}>
        <p className="text-sm font-medium text-slate-400">No stats content available</p>
      </div>
    );
  }
          ...
          <StatsEditableWrap editMode={editMode} target={{ type: "statsHeader" }} onEditTarget={onEditTarget}>
            {editMode ? (
              <div className="text-center max-w-4xl mx-auto mb-14">
                ...
                <h1 className="text-3xl sm:text-4xl md:text-5xl font-black mb-4 tracking-tight text-slate-900 leading-tight">
                  <span className="bg-clip-text text-transparent bg-gradient-to-r from-indigo-700 to-slate-800 block">{data.title}</span>
                </h1>
              ...
              >
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-6 bg-indigo-100 text-indigo-700 border border-indigo-200 backdrop-blur-sm shadow-sm">
                  <Sparkles className="w-3.5 h-3.5" /> {data.eyebrow || "Our Impact"}
                </div>
            ...
            {data.stats.map((stat, i) => {
              const cardContent = (
                <TiltCard 
                  editMode={editMode}
                  max={8}
                  className="relative p-5 sm:p-7 md:p-8 text-center bg-white/85 backdrop-blur-lg rounded-2xl border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.04)] hover:shadow-[0_16px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300"
                >
                  <div className="absolute -inset-0.5 rounded-2xl bg-gradient-to-br opacity-20 group-hover:opacity-35 transition-opacity blur-sm pointer-events-none" style={{ background: stat.color }} />
                  ...
                  <StatsEditableWrap editMode={editMode} target={{ type: "statsCard", index: i }} onEditTarget={onEditTarget} onDeleteTarget={onDeleteTarget} canDelete={data.stats.length > 1}>
                    ...
                    >
                      {i === 0 ? <Users className="w-6 h-6 sm:w-7 sm:h-7" /> :
                       i === 1 ? <Award className="w-6 h-6 sm:w-7 sm:h-7" /> :
                       i === 2 ? <Calendar className="w-6 h-6 sm:w-7 sm:h-7" /> :
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="text-xs sm:text-sm font-bold text-slate-700">{stat.label}</div>
                      <div className="text-[11px] sm:text-xs text-slate-500 mt-1">{stat.note}</div>
                    </div>
                  </StatsEditableWrap>
                </TiltCard>
              );
              ...
              <div className="relative rounded-3xl overflow-hidden shadow-xl bg-slate-900 aspect-[4/3] w-full group">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 z-20">
                  <div className="backdrop-blur-md bg-white/15 p-3.5 sm:p-4 rounded-2xl border border-white/25 inline-block shadow-lg">
                    <div className="text-white text-lg sm:text-xl font-bold">{data.story.imageTopTitle || "Our Campus"}</div>
                    <div className="text-white/80 text-xs sm:text-sm">{data.story.imageTopSubtitle || "Hetauda-2"}</div>
                  </div>
                </div>
              </div>
            </StatsEditableWrap>
            ...
            <div>
              <StatsEditableWrap editMode={editMode} target={{ type: "storyText" }} onEditTarget={onEditTarget}>
                <div className="flex items-center gap-2.5 mb-3 sm:mb-4">
                  <div className="w-7 h-7 rounded-full bg-amber-100 flex items-center justify-center text-amber-700"><Sparkles className="w-4 h-4" /></div>
                  <span className="text-xs sm:text-sm font-bold uppercase tracking-widest text-amber-700">{data.story.badge}</span>
                </div>
                <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 mb-4 sm:mb-6 tracking-tight leading-tight">{data.story.title}</h2>
          ...
          <div className="mb-16 sm:mb-24">
            <StatsEditableWrap editMode={editMode} target={{ type: "excellenceHeader" }} onEditTarget={onEditTarget}>
              <div className="text-center mb-10 sm:mb-12">
                ...
                <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900">Excellence in Every Subject</h2>
                <p className="text-slate-600 max-w-2xl mx-auto mt-2 text-sm sm:text-base">{data.excellence.description}</p>
              </div>
            </StatsEditableWrap>
              ...
              {editMode && (
                ...
                >
                  <div className="flex flex-col items-center gap-2 text-purple-600">
                    <Plus className="w-7 h-7 group-hover:scale-110 transition-transform" />
                    <span className="text-xs sm:text-sm font-bold">Add Excellence Card</span>
                  </div>
                </div>
              )}
            </div>
            ...
            >
              <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8 sm:mb-10">
                <div>
                  ...
                  <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-800">Latest Updates</h2>
                </div>
                <Link 
                  to="/notices"
              ...
              {notices.length === 0 ? (
                <div className="border border-indigo-200 rounded-2xl p-10 text-center bg-white/50 backdrop-blur-sm">
                  ...
                  <h3 className="text-lg font-bold text-slate-500">No notices available</h3>
                </div>
              ) : (
                <div className="space-y-3 sm:space-y-4 relative z-10">
                  {notices.map((notice, i) => {
                    const borderColors = ["border-indigo-400", "border-amber-400", "border-emerald-400"];
                    return (
                      <motion.div
                      ...
                      >
                        ...
                        >
                          <div className={`p-4 sm:p-5 md:p-6 rounded-2xl bg-white/70 backdrop-blur-sm border-l-4 ${borderColors[i % borderColors.length]} hover:bg-white/90 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg`}>
                            <div className="flex flex-col md:flex-row md:items-center gap-3 sm:gap-4">
                              <div className="flex-1 min-w-0">
                                ...
                                <h3 className="text-base sm:text-lg font-bold text-slate-800 line-clamp-1">{notice.title}</h3>
                                <p className="text-xs sm:text-sm text-slate-500 line-clamp-1 mt-0.5">{notice.description || "Click to read more."}</p>
                              </div>
          ...
          >
            ...
            >
              <div className="sticky top-0 z-10 bg-white/90 backdrop-blur-sm p-5 sm:p-6 border-b border-slate-100 flex justify-between items-start">
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-slate-900">{selectedNotice.title}</h2>
